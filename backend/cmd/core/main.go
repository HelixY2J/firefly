...
func main() {
	masterPort := 50051
	// Bind to all interfaces for accepting connections
	serverAddress := fmt.Sprintf("0.0.0.0:%d", masterPort)
	// But register with actual IP for discovery
	masterAddress := fmt.Sprintf("%s:%d", getOutboundIP(), masterPort)
	reg := registry.NewInMemoryRegistry(serverAddress)
	...
	relay.SetPlaybackHandler(func(filename string, status string) {
		...
		case "PLAY":
			...
			log.Printf("Starting playback of %s", filename)
		case "STOP":
			// Stop playing the audio file
			log.Printf("Stopping playback of %s", filename)
		}
	})
